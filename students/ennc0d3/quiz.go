...
func normStr(input string) string {
...
func readCSV(quizFd *os.File) ([]Quiz, error) {
	...
	for _, a := range records {
		quizes = append(quizes, Quiz{a[0], a[1], ""})
	}
	return quizes, nil
}
...
func runQuiz(quizes []Quiz, stopCh <-chan bool, resultCh chan<- int) {
	...
	go func() {
		for i, _ := range quizes {
			...
			if normStr(quizes[i].Response) == normStr(quizes[i].Expected) {
				correct++
			}
			resultCh <- correct
		}
...
func showQuizResults(quiz []Quiz, correct int) {
	...
	for _, q := range quiz {
		fmtStr := "%s = %s %c\n"
		if normStr(q.Response) != normStr(q.Expected) {
